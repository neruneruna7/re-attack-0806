...
def load_saved_denorm_image(epsilon: float, index: int) -> Tensor:
    """
...
def reattack_saved_image(epsilon_saved: float, index: int, re_eps: float, steps: int = 1, save_out: bool = True):
    """
    ...
    img_denorm = load_saved_denorm_image(epsilon_saved, index)  # [1,1,H,W] on device
...
before, after, adv_tensor = reattack_saved_image(0.05, 0, re_eps=0.05, steps=1, save_out=True)
