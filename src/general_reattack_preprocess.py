# 攻撃 -> 再攻撃 -> 前処理 を実行し、その結果を評価・保存する汎用コード
...
class Runner:
    ...
    def run(self) -> Iterator[AttackResult]:
        ...
        for data, target in tqdm(self.test_loader, desc="Preprocess & Re-Attack"):
            ...
            reattacked_data_ts_norm = self._perform_attack(attacked_data_ts_norm, reattack_target, self.cfg.reattack_kind, self.cfg.reattack_params, is_reattack=True)
            ...
            preprocessed_data_ts_norm = self._perform_preprocessing(reattacked_data_ts_norm)
